...
const demoUsers = [
    ...
    { role: 'Waiter', email: 'waiter@pos.com', password: 'waiter123' },
    { role: 'Cashier', email: 'cashier@pos.com', password: 'cashier123' },
    { role: 'Kitchen Staff', email: 'kitchen@pos.com', password: 'kitchen123' }
];
...
const handleLogin = async () => {
    ...
    if (result.success) {
        ...
        switch (authStore.userRole) {
            case 'waiter':
                webSocketService.subscribeAsWaiter(authStore.user.id);
                break;
            case 'kitchen_staff':
                webSocketService.subscribeAsKitchenStaff();
                break;
            case 'manager':
            case 'admin':
                webSocketService.subscribeAsManager();
                break;
            case 'cashier':
                webSocketService.subscribeAsCashier();
                break;
        }
